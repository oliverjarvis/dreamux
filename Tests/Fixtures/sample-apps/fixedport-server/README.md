...
Dreamux test fixture. A python3 stdlib HTTP server that binds a
...
`[[runners]]` entry in `.dreamux/run.toml`. Tests assert on these
exact strings — change them in both places or not at all.

`dreamux-runner.snippet.toml` is the `[[runners]]` block the shim
concatenates into `.dreamux/run.toml` during detect; `{{CWD}}` is
