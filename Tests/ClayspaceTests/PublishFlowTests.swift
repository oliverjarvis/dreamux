...
final class PublishFlowTests: XCTestCase {
    ...
    /// Whatever CLAYSPACE_GH_BIN held before this suite pointed it at
    ...
    override func setUp() async throws {
        ...
        previousGhBin = ProcessInfo.processInfo.environment["CLAYSPACE_GH_BIN"]
        setenv("CLAYSPACE_GH_BIN", fakeGhPath, 1)
    ...
    override func tearDown() async throws {
        if let previousGhBin {
            setenv("CLAYSPACE_GH_BIN", previousGhBin, 1)
        } else {
            unsetenv("CLAYSPACE_GH_BIN")
        }
        previousGhBin = nil
        sandbox?.destroy()
        sandbox = nil
    ...
    ///   • a Clayspace bare+worktree clone inside the project,
    ...
    func testInitializeStatesReportsGhMissingWhenBinaryAbsent() async throws {
        ...
        setenv("CLAYSPACE_GH_BIN", sandbox.root.appendingPathComponent("missing-gh").path, 1)
