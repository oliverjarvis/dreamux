...
/// Renders one tab's content, dispatched by kind. Deliberately its own
/// file and `internal` rather than private to `WorkspaceTerminalContainer`:
/// a pip panel hosts THIS view, so a pipped tab and a paned tab are the
/// same view in two places — never two renderers to keep in step.
