...
struct TranscriptItem: Identifiable, Sendable {
    ...
    enum Kind: Sendable {
        ...
        case toolUse(name: String, input: String)     // input = pretty JSON
    ...
    static func parse(_ text: String) -> [TranscriptItem] {
        ...
        for line in text.split(separator: "\n", omittingEmptySubsequences: true) {
            ...
            guard let object = try? JSONSerialization.jsonObject(with: data),
                ...
                items.append(TranscriptItem(kind: .raw(String(line))))
                continue
            }
            switch dict["type"] as? String {
            case "user":
                appendMessage(dict["message"] as? [String: Any], isUser: true, into: &items)
            case "assistant":
                appendMessage(dict["message"] as? [String: Any], isUser: false, into: &items)
            case "summary":
                if let summary = dict["summary"] as? String, !summary.isEmpty {
                    items.append(TranscriptItem(kind: .summary(summary)))
                }
            case let type? where skipTypes.contains(type):
                break
            default:
                items.append(TranscriptItem(kind: .raw(pretty(dict) ?? String(line))))
            }
        }
        return items
    }
        ...
        if let text = content as? String {
            ...
            for block in blocks {
                ...
                case "tool_use":
                    ...
                    items.append(TranscriptItem(kind: .toolUse(name: name, input: input)))
                case "tool_result":
