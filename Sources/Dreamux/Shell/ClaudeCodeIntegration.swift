...
/// shim at `Dreamux.app/Contents/Resources/bin/`, and `PTYShellSession`
/// prepends that directory to every spawned shell's `PATH`. When the
/// user types `claude` in a Dreamux tab they hit our shim, which
/// resolves the real binary and `exec`s it with an inline `--settings`
/// JSON that wires Stop / Notification hooks through `dreamux-hook`.
///
/// Outside Dreamux, `claude` keeps resolving to the real binary, so
...
enum ClaudeCodeIntegration {
    ...
    /// Path to the bundled `dreamux-hook` script.
    static var hookExecutablePath: String? {
        shimDirectory?.appendingPathComponent("dreamux-hook").path
            .nonEmptyIfExists
    }
    ...
    /// `DREAMUX_CLAUDE_BIN` to a deterministic fake so Detect /
    ...
    static var claudeInvocation: String {
        if let override = ProcessInfo.processInfo.environment["DREAMUX_CLAUDE_BIN"],
           !override.isEmpty {
