...
struct RunSetupView: View {
    ...
    private var headerSubtitle: String {
        ...
        return "Have Claude figure out how to start and stop each repo, then save it as .clayspace/run.toml."
    }
    ...
    private var detectAction: some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            if repoStore.repositories.isEmpty {
                ...
                Text("Claude will inspect each repo and write .clayspace/run.toml.")
                    .font(.caption2)
                    .foregroundStyle(.tertiary)
                    .fixedSize(horizontal: false, vertical: true)
            }
    ...
    private func sendClaude(_ prompt: String) {
        ...
        do {
            let dir = FileManager.default.temporaryDirectory
                .appendingPathComponent("clayspace-prompts", isDirectory: true)
            ...
            command = claudeCommand("$(cat \(shellQuote(file.path)))", quoted: false)
        } catch {
            // Fall back to inline typing — long-prompt hazards beat
            // not sending anything at all.
            command = claudeCommand(prompt)
        }
        Task {
    ...
    /// for its quoting.
    private func claudeCommand(_ argument: String, quoted: Bool = true) -> String {
    ...
    private func detectPrompt() -> String {
        ...
        port_env matters: Clayspace runs the same app from multiple git worktrees at once \
        ...
        port belongs and Clayspace substitutes each instance's actual port. A URL opens in \
    ...
    private func isolatePrompt(for runner: ParsedRunner) -> String {
        return """
        In this project's .clayspace/run.toml, the runner named "\(runner.name)" currently binds a fixed port \
        ...
        Then update the matching [[runners]] entry in .clayspace/run.toml to add: \
    ...
    private func diagnosePrompt(for runner: ParsedRunner) -> String {
        ...
        Figure out why and fix it — install missing deps, correct the start command in .clayspace/run.toml, \
