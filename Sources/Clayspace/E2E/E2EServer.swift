...
/// on the path from `CLAYSPACE_E2E_SOCKET`, speaks newline-delimited
...
final class E2EServer: @unchecked Sendable {
    ...
    /// Bind and start accepting. Called once from `ClayspaceApp.init`
    ...
    static func start(socketPath: String) {
        ...
        guard server.listenOnSocket() else {
            FileHandle.standardError.write(
                Data("[clayspace-e2e] failed to bind \(socketPath): \(String(cString: strerror(errno)))\n".utf8)
            )
            return
        }
        shared = server
    ...
    private let queue = DispatchQueue(label: "com.clayspace.e2e-server", qos: .userInitiated)
