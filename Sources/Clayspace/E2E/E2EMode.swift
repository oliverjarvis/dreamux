...
/// `Sources/Clayspace/E2E/` keys off `isActive`, which is true only
/// when the launcher set `CLAYSPACE_E2E_SOCKET` — so a normal user
...
enum E2EMode {
    ...
    static var socketPath: String? {
        guard let value = ProcessInfo.processInfo.environment["CLAYSPACE_E2E_SOCKET"],
    ...
    /// Folder name (under `CLAYSPACE_PROJECTS_ROOT`) of a project the
    ...
    static var autoOpenProjectName: String? {
        guard let value = ProcessInfo.processInfo.environment["CLAYSPACE_E2E_AUTOOPEN"],
              !value.isEmpty else { return nil }
        return value
    }
}
